...
Rappel : une classe de tests est basée sur l'isolation des tests, en terme de propriétés définies dans la classe de test. Chaque test repart avec ces propriété ré-initialisée. Vous devez systématiquement garder ce principe d'isolatio de vos tests dans l'analyse algorithmique de vos classes métiers.
